...
CONTAINS
   ...
   SUBROUTINE GETVAR_2D_R4(idx_f, idx_v, cf_in, cv_in, Nt, kz, kt, X, jt1, jt2, Nk)
      ...
      IF( (idx_f==0).AND.(idx_v==0) ) CALL print_err(crtn, ' PROBLEM #2 file and variable handle never created => &
                                         &           '//TRIM(cv_in)//' in '//TRIM(cf_in))
   ...
   SUBROUTINE P2D_MAPPING_AB(cf_out, xlon, xlat, imtrcs, ralfbet, vflag, id_pb,  d2np)
      ...
      CALL sherr( NF90_PUT_ATT(id_f, NF90_GLOBAL, 'Info', & 
         &        'File containing mapping/weight information for bilinear interpolation with SOSIE.'), &
         &        crtn,cf_out,cdum)
   ...
   FUNCTION to_epoch_time_scalar( cal_unit_ref0, rt, dt )
      ...
      DO ipass=1, nb_pass
         ...
         DO WHILE ( lcontinue )
            ...
            !
            !IF( jd /= jd_old ) THEN
            !   WRITE(*,'(" ***  now : ",i4,"-",i2.2,"-",i2.2," ",i2.2,":",i2.2,":",i2.2," s cum =",i," d cum =",i)') & 
            !       & jy, jmn, jd, jh, jm, js,  rjs_t
            !END IF
            IF( (zt <= rjs_t).AND.(zt > rjs_t_old) ) lcontinue = .FALSE.
            IF( jy == 2019 ) THEN
               PRINT *, 'rjs_t =', rjs_t
         ...
         !
         !PRINT *, 'Found !!!', zt, REAL(rjs_t)
         WRITE(*,'(" *** to_epoch_time_scalar => Date : ",i4,"-",i2.2,"-",i2.2," ",i2.2,":",i2.2,":",i2.2)') & 
             & jy, jmn, jd, jh, jm, NINT(rjs)
         !PRINT *, ' + rjs0_epoch =', rjs0_epoch
   ...
   SUBROUTINE to_epoch_time_vect( cal_unit_ref0, vt,  l_dt_below_sec )
      ...
      DO jt = 0, ntr ! 0 is the initial pass to find the start
         ...
         !
         IF( jt==1   ) WRITE(*,'(" *** to_epoch_time_vect => Start date : ",i4,"-",i2.2,"-",i2.2," ",i2.2,":", & 
                           & i2.2,":",i2.2,":",i2.2," epoch: ",f15.4)') jy, jmn, jd, jh, jm, js,jx, vtmp(MAX(jt,1))
         !WRITE(*,'(" *** to_epoch_time_vect: jt =",i8.8," => ",i4,"-",i2.2,"-",i2.2," ",i2.2,":",i2.2,":",i2.2, & 
         !    & ":",i2.2," epoch: ",f15.4)') jt, jy, jmn, jd, jh, jm, js,jx, vtmp(MAX(jt,1))
         IF( jt==ntr ) WRITE(*,'(" *** to_epoch_time_vect =>   End date : ",i4,"-",i2.2,"-",i2.2," ",i2.2,":", & 
                           & i2.2,":",i2.2,":",i2.2," epoch: ",f15.4)') jy, jmn, jd, jh, jm, js,jx, vtmp(MAX(jt,1))
